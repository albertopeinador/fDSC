...
try:
    ...
    for i in range(len(temps)):
        ...
        if regs_label not in st.session_state:
            ...
            left, right = fai.find_int_region(big_data[i], int_dif_th*(big_data[i][0]['Heat Flow'] - big_data[i][1]['Heat Flow']).max(), 'Heat Flow')
            #   Old code ------>    regs, left, right = fai.find_int_region(big_data[i], int_dif_th*(big_data[i][0]['Heat Flow'] - big_data[i][1]['Heat Flow']).max(), 'Heat Flow')

            #   Append limits to dictionary as a tuple
            #   Old code ------>    lims[temps[i]] = (left, right)
            #   Old code ------>    int_regs[temps[i]] = regs
