...
def load_files(main_dir, temps, temp_dict, cutoff):
    ...
    for i in temps:
        ...
        df = pd.read_csv(abs_path.replace('.txt', '_modified.txt'), sep = '\t', encoding = 'unicode_escape', index_col = 'Index', skipinitialspace = True)
        df_ref = pd.read_csv(abs_path_ref.replace('.txt', '_modified.txt'), sep = '\t', encoding = 'unicode_escape', index_col = 'Index', skipinitialspace = True)
