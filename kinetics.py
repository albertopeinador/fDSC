...
def get_names(og_file):  #   Modify column names
    names = list(
        [f"curva {i}" for i in range(count_columns(og_file))]
    )  #   Create default names
    names[0] = "time"  #   Set first column to time by default
    tableinput = st.toggle(
        "Custom names"
    )  # Ask for name input type: By default: table editor, option for text input
    if not tableinput:
        editnames = pd.DataFrame(
            [names], columns=names
        )  #  DataFrame so it can be edited with st.data_editor
        newnames = st.data_editor(
            editnames, use_container_width=True, hide_index=True
        )  # Widget to edit data as table
        edited_names = newnames.iloc[0].astype(str).tolist()  # Turn it back into list
    else:
        entered_names = st.text_area(
            "Enter the column of names for each curve", placeholder="\n".join(names[1:])
        )
        edited_names = entered_names.splitlines()
        if len(names[1:]) != len(edited_names):
            st.write(f"Using default names.")
            #st.write(f"Enter appropriate list or modify using \'Custom names\' toggle for custom names.")
            st.write(f'Looking for {len(names[1:])} names, {len(edited_names)} were given')
...
def kinetics(df, filename):  # Proper processing
    ...
    left, right = st.columns(
        [2, 3]
    )  # Main columns for the UI, left for controls and integrals plot and right for plot
    ...
    for i in names[1:-1]:
        ...
        max_search_min = (
            df[f"{i} subtracted"]
            .where(df[names[0]] >= st.session_state[f"max_rangesearch {i}"][0])
            .first_valid_index()
        )

        max_search_max = (
            df[f"{i} subtracted"]
            .where(df[names[0]] >= st.session_state[f"max_rangesearch {i}"][1])
            .first_valid_index()
        )
        ...
        if f"{i} max" not in st.session_state:
            max_peak_index = (
                df[f"{i} subtracted"].iloc[max_search_min:max_search_max].idxmax()
            )
    ...
    for i in names[1:-1]:
        ...
        if f"t_index {i}" not in st.session_state:
            st.session_state[f"t_index {i}"] = (
                df[names[0]]
                .where(df[names[0]] >= st.session_state[f"{i} rightlim"])
                .first_valid_index()
            )
        ...
        st.session_state[f"t_index {i}"] = (
            ...
            len(df[names[0]]) - 1, st.session_state[f"t_index {i}"]
        )  # Making sure not to go over last data point
        ...
        condicion = (
            df[f"{i} subtracted"].iloc[:end] - st.session_state[f"baseline {i}"] < 0
        )

        #   Initialize left limit index value
        if f"start {i}" not in st.session_state:
            st.session_state[f"start {i}"] = (
                df[f"{i} subtracted"].iloc[:end].where(condicion).last_valid_index()
            )
        ...
        if f"signal_segment {i}" not in st.session_state:
            st.session_state[f"signal_segment {i}"] = df[f"{i} subtracted"].iloc[
                st.session_state[f"start {i}"] : end + 1
            ]
        ...
        if f"time_segment {i}" not in st.session_state:
            st.session_state[f"time_segment {i}"] = df[names[0]].iloc[
                st.session_state[f"start {i}"] : end + 1
            ]
        ...
        area_max_peak = np.trapz(
            st.session_state[f"signal_segment {i}"] - st.session_state[f"baseline {i}"],
            st.session_state[f"time_segment {i}"],
        )
    ...
    currentfig.add_trace(
        go.Scatter(
            x=st.session_state[f"time_segment {curve}"],
    ...
    currentfig.add_trace(
        go.Scatter(
            x=st.session_state[f"time_segment {curve}"],
            y=[st.session_state[f"baseline {curve}"]]
            * len(st.session_state[f"time_segment {curve}"]),
            fill="tonexty",
            fillcolor="#cccccc",
            showlegend=False,
            line=dict(color="rgba(0,0,0,0)"),
    ...
    integrals_df = pd.DataFrame(
        list(reversed(integraciones[:])),
        index=list(reversed([i.replace(",", ".") for i in names[1:-1]])),
        columns=["Integral"],
    )
